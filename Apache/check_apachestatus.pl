...
#############################################################
# 20150722 <public at glam dot com> v1.7
#          Added CPU load support
#
...
my $output = " $timeelapsed seconds response time. Idle $IdleWorkers, busy $BusyWorkers, open slots $CountOpenSlots, Cpu Usage $CpuUsage%";
# add the performance data separator
  $output = $output." | ";
# add the performance data
  $output = $output."'Waiting_for_Connection'=$ConnectionWait 'Starting_Up'=$ConnectionStart 'Reading_Request'=$ConnectionRead ";
  $output = $output."'Sending_Reply'=$ConnectionReply 'Keepalive_read'=$ConnKeepalive 'DNS_Lookup'=$ConnDnsLookup ";
  $output = $output."'Closing_Connection'=$ConnectionClose 'Logging'=$ConnectionLog 'Gracefully_finishing'=$ConnGraceFinish ";
  $output = $output."'Idle_cleanup'=$ConnIdleCleanup 'Open_slot'=$CountOpenSlots";
  $output = $output." 'Cpu_Usage'=$CpuUsage";
...
# check if server-status has "ExtendedStatus On" output available
  if( defined($rPerSec) && defined($rPerSecSfx) && defined($bPerSec)
   && defined($bPerSecSfx) && defined($bPerReq) && defined($bPerReqSfx) ) {
    $output = sprintf("%s 'Requests_sec'=%0.1f '%s_per_sec'=%0.1f%s '%s_per_Request'=%0.1f%s\n",
